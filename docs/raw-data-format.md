...
New captures can be grouped in versioned session directories with checksums and
runtime completion metadata. See [session capture and replay](session-replay.md)
for commands, validation, incomplete-source policy and canonical hash encoding.
JSONL records must end with a newline, including the final record.
