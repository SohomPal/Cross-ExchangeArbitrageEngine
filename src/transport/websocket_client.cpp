...
struct WebSocketClient::Impl : std::enable_shared_from_this<Impl> {
    ...
    Impl(asio::io_context& io, asio::ssl::context& ssl, MessageHandler m, ErrorHandler e,
         std::string h, std::string p, std::function<void()> c, std::string t,
        ...
        requests = std::move(r);
        if (requests.empty())
            throw std::invalid_argument("WebSocket subscriptions required");
    }
    void abort_socket() {
        Error ignored;
            ...
            beast::get_lowest_layer(self->ws).async_connect(endpoints, [self](Error ec, auto) {
                ...
                self->ws.next_layer().async_handshake(
                    asio::ssl::stream_base::client, [self](Error ec) {
                        ...
                        self->ws.async_handshake(self->host, self->target, [self](Error ec) {
                            if (self->stopping)
                                return;
                            if (ec)
    ...
    void subscribe(std::size_t index) {
        writing = true;
        ws.async_write(asio::buffer(requests[index]),
...
WebSocketClient::WebSocketClient(asio::io_context& io, asio::ssl::context& ssl, MessageHandler m,
                                 ErrorHandler e, std::string host, std::string port,
                                 std::function<void()> connected, std::string target,
                                 std::vector<std::string> requests)
    : impl_(std::make_shared<Impl>(io, ssl, std::move(m), std::move(e), std::move(host),
                                   std::move(port), std::move(connected), std::move(target),
                                   std::move(requests))) {}
WebSocketClient::~WebSocketClient() = default;
void WebSocketClient::connect() { impl_->connect(); }
void WebSocketClient::close() { impl_->close(); }
void WebSocketClient::abort() { impl_->abort(); }
} // namespace transport
